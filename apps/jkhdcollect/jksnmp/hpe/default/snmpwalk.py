...
class WalkApi(object):
    ...
    def get_data(self):
        ...
        for lable in oid_lable_num():
            data_out = self.walk_cmd(self._lable2num(lable)) if self.version == '2c' \
            ...
            for line_data in data_out.split("\n"):
                oid_value = line_data.split('=')[1].split(': ')[-1].strip('"')
            ...
            if class_key in ['ip', 'power', 'cpu', 'memory', 'fan', 'disk']:
                if lable == 'ifDescr':
                    ...
                    for num, cpu_name in enumerate(line_list):
                        cpu_dict = dict()
                        cpu_dict['version'] = tmp_dict['processorDeviceVersionName'][num]
                ...
                #         result_dict['fan'][fan_name] = fan_dict
                    ...
                    for num, disk_name in enumerate(line_list):
                        ...
                        disk_dict['type'] = tmp_dict['physicalDiskMediaType'][num]
                        result_dict['disk'][num] = disk_dict
                else:
                    pass
            elif class_key == 'sys_uptime':
                result_dict['sys_uptime'] = line_list[0].split(') ')[-1]
            else:
                result_dict[class_key] = line_list[0]
...
if __name__ == '__main__':
    snmpwalk = WalkApi('xxx', '2c', '12.118.2.217', '161', 'hdinfo')
