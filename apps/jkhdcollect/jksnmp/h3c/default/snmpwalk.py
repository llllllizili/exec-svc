...
class WalkApi(object):
    ...
    def get_basic_info(self, lable, output):
        ...
        if_state_dict = dict()        
        if lable == 'sysName':
            basic_info_dict['sys_name'] = output.split(': ')[-1].replace('"','')
        elif lable == 'sysDesc':
            basic_info_dict['ios_version'] = output.split(': ')[-1]
        elif lable == 'sysUptime':
            basic_info_dict['sys_uptime'] = output.split(')')[-1].strip()
        elif lable == 'phySerialNum':
    ...
    def get_data(self):
        ...
        if self.intention == 'topinfo':
            rem_info = self.walk_cmd(self.oid_dict['lldpRemChassisId']) if self.version == '2c' else \
                    self.walk_cmd_v3(self.oid_dict('lldpRemChassisId'))
            if 'Timeout' in rem_info or 'No Such' in rem_info:
                ...
                for item in rem_info.split('\n'):
                    if 'hex' in item.lower():
                        ...
                        if port_num in ifname_dict.keys():
