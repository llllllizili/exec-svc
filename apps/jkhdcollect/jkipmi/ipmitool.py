...
class IpmiApi:
    def __init__(self, host, user, passwd, port):
        self.passwd = passwd
        self.host = host
        self.base_info = base_info(host=self.host, user=user, passwd=self.passwd, port=port)
        self.hardware_info = hardware_info(host=self.host, user=user, passwd=passwd, port=port)
    ...
    def ipmi_verify(self):
        ...
        if self.passwd == '__jkstack_decrypt_error':
            return False, 'password not encrypt'

        data_out = self.ipmirun(self.base_info['mc']).decode('utf-8')

        if 'Error' in data_out:
            data_dict['status'] = 'FAILURE'
            data_dict['result'] = data_out
            jklog('error', data_out)
            return False, data_dict
        else:
            data_dict['result'] = "verify OK"
