...
get_app(){
  local IFS=$'\n'
  local OLDIFS="$IFS"
  SS=`ss -ntulp  2>/dev/null | grep -E 'tcp|udp' |awk '{print $5,$7}'`
# [::]:5672 users:(("docker-proxy",pid=25912,fd=4))
  echo -e "[\c"
  for line in $SS
    do
      SERVER=`echo $line | awk '{print $2}' | awk -F '"' '{print $2}'`
      SERVER_PORT=`echo $line | awk '{print $1}' | awk -F: '{print $NF}'`
      SERVER_PID=`echo $line | awk '{print $2}' | awk -F ',' '{print $2}' | awk -F '=' '{print $2}'`
      if [ $os_release == 'centos6' ]; then
        SERVER_PID=`echo $line | awk '{print $2}' | awk -F ',' '{print $2}'`
      fi
      if [ -n "$SERVER_PID" ]; then
        echo -e "{\"server\":\"$SERVER\",\"port\":\"$SERVER_PORT\",\"pid\":\"$SERVER_PID\"}"
      fi
    done | sed '$!s/$/,/'
    echo -e "]\c"
}
