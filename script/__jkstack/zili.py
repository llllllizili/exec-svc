...
for option, value in options:	
	...
	if option in ("--module"):
		...
		# 模块缓存统一放模块目录
		TF_BASE_MODULE_CACHE=TF_CACHE+TF_BASE_MODULE+'.terraform'  # 父模块
...
def exec_tf_init():
	docker_tf_init = "docker run --rm -v {}:{} -v {}:{} -w {} -e ALICLOUD_ACCESS_KEY={} -e ALICLOUD_SECRET_KEY={} -e ALICLOUD_REGION={} hashicorp/terraform:1.0.9 {} -var-file={}".format(
		TF_MODULE,TF_MODULE,
		TF_BASE_MODULE_CACHE,TF_BASE_MODULE_CACHE,
		TF_MODULE,
		accessKey,
		secretKey,
		region,
...
def exec_tf_create():
	docker_tf_apply = "docker run --rm -v {}:{} -v {}:{} -w {} -e ALICLOUD_ACCESS_KEY={} -e ALICLOUD_SECRET_KEY={} -e ALICLOUD_REGION={} hashicorp/terraform:1.0.9 {} -var-file={}".format(
		TF_MODULE,TF_MODULE,
		TF_BASE_MODULE_CACHE,TF_BASE_MODULE_CACHE,
		TF_MODULE,
		accessKey,
...
#  python3 __jk_script_dpa_tf_aliyun_ecs.py --module 'aliyun/ecs' --accessKey 'LTAI5t5gn******' --secretKey '3sdtxM*****DMAMxrO' --region 'cn-hangzhou' --imageIds '["centos_7_7_64_20G_alibase_20191008.vhd"]' --instanceType 'ecs.xn4.small' --systemDiskCategory 'cloud_efficiency' --systemDiskSize '50' --numberOfInstances '1' --instanceName 'dpa-test' --instancePassword '123qweASD' --securityGroups '["sg-bp1fhu45pdvomvhg0isp"]' --vswitchId 'vsw-bp1gx9q8egwdw6b7ltus6' --instanceChargeType 'PostPaid'