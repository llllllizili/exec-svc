...
get_app(){
  ...
  for line in $SERVER
    ...
    do
      ...
      SS=`ss -ntulpa  2>/dev/null | grep -E 'tcp|udp' |awk '{print $1,$2,$5,$6,$7}' | grep $PID`

      for p_line in $SS
        do
          APP_PORT=`echo "$p_line" | grep $PID | awk '{print $3}' | awk 'END{print $1}'|awk -F: '{print $(NF)}'`
          STATE=`echo "$p_line" | grep $PID | awk '{print $2,$5}' | awk 'END{print $1}'`
          NUM=`echo "$p_line" | grep $PID | wc -l`
