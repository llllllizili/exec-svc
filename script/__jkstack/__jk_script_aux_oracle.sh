...
# 进程,应用详情
# AUX oracle进程过去去重 - 定义去重进程安装目录
oracle_process='/oracle/u01/app/oracle/product/11.2.0.4/db_1/dbs'
...
get_app(){
  ...
  SS=`ss -npa  2>/dev/null | grep -E 'tcp|udp' |awk -F' ' '{print $1,$2,$5,$6,$7}'`
  echo -e "[\c"
  for line in $SERVER
    do
      #echo $line
      ...
      APP_PORT=`echo "$SS" | grep $PID | awk '{print $3}' | awk 'END{print $1}'|awk -F: '{print $2}'`
      ...
      # if [[ $P_PID -eq '0' ]] || [[ $P_PID -eq '1' ]] || [[ $P_PID -eq '2' ]] || [[ $NUM -eq '0' ]]; then
        ...
        if [[ $oracle_process == $INSTALL_DIR ]]; then
          if [[ $oracle_num == 'true' ]]; then
