...
get_relations(){
  #IFS=$'\n'
  #IFS='\n'
  app="rmsock : Unable to read kernel address"
  OLDIFS="$IFS"
  IFS='
'
  ...
  do
    ...
    echo "{\"app\":\"$app\",\"protocol\":\"$protocol\",\"local_addr\":\"$localAddr\",\"local_port\":\"$localPort\",\"foreign_addr\":\"$foreignAddr\",\"foreign_port\":\"$foreignPort\",\"state\":\"$state\"}"
  done | sed '$!s/$/,/'
  echo  "]\c"
  IFS="$OLDIFS"
}
